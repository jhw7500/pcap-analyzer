    ...
    **중복 제거**: 같은 로밍이 `wifi_roam.py`의 두 지점에서 각각 로그를 남긴다
    ...
    for ln in lines:
        ...
        if _LOG_SKIP.search(ln.msg):
            ...
            # 로밍에 남의 근거가 붙는다. 앵커를 소비 후 폐기하는 로밍 짝짓기와
            # 같은 규칙이다.
            #
            # **현 데이터에서는 발현하지 않는 잠재 결함이다.** 실측 로그의 스킵
            # 6/18/4건 뒤 첫 로밍은 3/9/2건인데, 그 전부가 자기 condition·candidate를
            # 새로 남겨 낡은 값이 덮어써진다(수정 전/후 파서 대조 결과 918건 중
            # 근거가 달라진 로밍 0건). 드라이버가 근거 줄을 빠뜨리는 경우에만
            # 드러나므로, 발현을 기다리지 않고 상태를 닫아둔다.
            last_cond = None
            last_cand = None
            continue
