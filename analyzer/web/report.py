...
def _format_epoch(epoch: Any) -> str:
    """epoch 초 → ISO-ish 'YYYY-MM-DD HH:MM:SS'. 실패 시 빈 문자열."""
    try:
        from datetime import datetime, timezone
        return datetime.fromtimestamp(float(epoch), tz=timezone.utc).astimezone().strftime(
            "%Y-%m-%d %H:%M:%S"
        )
...
def _correlations_section(diagnosis: Dict[str, Any]) -> List[str]:
    ...
    for n, c in enumerate(valid, start=1):
        ...
        n_refs = len(c.get("frame_refs") or [])
        if n_refs:
...
def _issues_table(diagnosis: Dict[str, Any]) -> List[str]:
    ...
    for iss in issues:
        ...
        action = (iss.get("action") or iss.get("recommendation") or "").replace("|", "\\|")
