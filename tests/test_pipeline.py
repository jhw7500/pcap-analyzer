...
from analyzer.pipeline import (
    _structured_overview, _structured_signal, _structured_ping,
    _structured_roaming, _structured_per_second, _structured_device_stats,
...
class TestStructuredPing:
    def test_matched_pair(self):
        frames = [
            make_frame(number=1, epoch=1000, icmp_type="8", ip_src="10.0.0.1",
                       ip_dst="10.0.0.2", icmp_seq="1", ta=STA1, ra=AP1),
            make_frame(number=2, epoch=1000.005, icmp_type="0", ip_src="10.0.0.2",
                       ip_dst="10.0.0.1", icmp_seq="1", ta=AP1, ra=STA1),
        ]
    ...
    def test_seq_reuse_time_window(self):
        ...
        frames = [
            ...
            make_frame(number=2, epoch=1000.005, icmp_type="0", ip_src="10.0.0.2",
                       ip_dst="10.0.0.1", icmp_seq="1", ta=AP1, ra=STA1),
            # 60초 뒤 seq=1 재사용
            make_frame(number=3, epoch=1060, icmp_type="8", ip_src="10.0.0.1",
                       ip_dst="10.0.0.2", icmp_seq="1", ta=STA1, ra=AP1),
            make_frame(number=4, epoch=1060.010, icmp_type="0", ip_src="10.0.0.2",
                       ip_dst="10.0.0.1", icmp_seq="1", ta=AP1, ra=STA1),
        ]
        result = _structured_ping(frames, SAMPLE_ROLES)
    ...
    def test_request_outside_window_treated_as_loss(self):
        ...
        frames = [
            make_frame(number=1, epoch=1000, icmp_type="8", ip_src="10.0.0.1",
                       ip_dst="10.0.0.2", icmp_seq="99", ta=STA1, ra=AP1),
            make_frame(number=2, epoch=1031, icmp_type="0", ip_src="10.0.0.2",
                       ip_dst="10.0.0.1", icmp_seq="99", ta=AP1, ra=STA1),
        ]
        result = _structured_ping(frames, SAMPLE_ROLES)
        assert len(result["pairs"]) == 0
        assert len(result["losses"]) == 1
    ...
    def test_duplicate_reply_only_first_matches(self):
        ...
        frames = [
            make_frame(number=1, epoch=1000, icmp_type="8", ip_src="10.0.0.1",
                       ip_dst="10.0.0.2", icmp_seq="7", ta=STA1, ra=AP1),
            make_frame(number=2, epoch=1000.005, icmp_type="0", ip_src="10.0.0.2",
                       ip_dst="10.0.0.1", icmp_seq="7", ta=AP1, ra=STA1),
            make_frame(number=3, epoch=1000.010, icmp_type="0", ip_src="10.0.0.2",
                       ip_dst="10.0.0.1", icmp_seq="7", ta=AP1, ra=STA1),
        ]
        result = _structured_ping(frames, SAMPLE_ROLES)
        assert len(result["pairs"]) == 1
    ...
    def test_reply_without_request_ignored(self):
        # reply만 있고 대응 req 없음
        frames = [
            make_frame(number=1, epoch=1000, icmp_type="0", ip_src="10.0.0.2",
                       ip_dst="10.0.0.1", icmp_seq="5", ta=AP1, ra=STA1),
        ]
        result = _structured_ping(frames, SAMPLE_ROLES)
        assert len(result["pairs"]) == 0
        assert len(result["losses"]) == 0
